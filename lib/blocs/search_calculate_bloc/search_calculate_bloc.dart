...
class SearchCalculateBloc
    extends Bloc<SearchCalculateEvent, SearchCalculateState> {
  ...
  SearchCalculateBloc(
      {required this.catsRepository, required this.searchCatBloc})
      : super(SearchCalculateState.initial()) {
    catsSearchSubscription =
        searchCatBloc.stream.listen((SearchCatState searchCatState) {
      _setFilterCats();
    });
    ...
    on<SearchCalculateFiltertEvent>((event, emit) {
      emit(state.copyWith(
          ...
          status: CatsStatus.complete,
          searchTerm: event.termLetter));
    });
  }
